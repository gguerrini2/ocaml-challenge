...
A list is said to be perforated if, for every pair of consecutive elements in the list, there exists at least one integer strictly between 
them.
...
| [] -> true
| x::y::rest when abs(y-x)>=2 -> is_perforated (y::rest)
| x::[] -> true
| _ -> false;;  
